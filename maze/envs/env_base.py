...
class Environment(gym.Env):
    ...
    def __init__(self, task):
        ...
        self._should_restart = True  
        self.observation_type = "position"
        self.action_space = gym.spaces.Discrete(self._task.action_spec.n)
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(task.maze.height,task.maze.width,3))
    ...
    def reset(self, seed=None, options=None):
        ...
        obs_info = {
            'position': obs.agent.position,
            'index': obs.agent.index,
            'goal': obs.goal.position,
        }
        obs = obs.agent.image
        return obs, obs_info

    def step(self, action):
        assert self._should_restart == False
        ...
        obs = obs.agent.image
        return obs, reward, is_final, is_last, obs_info
